...
from scripts.run_cemtad import run_pipeline  # noqa: E402
...
def autodetect_ts_col(df_head: pd.DataFrame) -> Optional[str]:
    # Prefer column literally named "Time"
    for c in df_head.columns:
        if str(c).lower() == "time":
            return c
    # Otherwise, best-effort by parsing success ratio
    best = None
    best_ok = 0.0
    for c in df_head.columns:
        ok = pd.to_datetime(df_head[c], errors="coerce").notna().mean()
        if ok > best_ok:
            best_ok, best = ok, c
    return best
    ...
    if gap > 0 and segs:
        ...
        for s in segs[1:]:
            ...
            if s[0] - ps[1] - 1 <= gap:
                # merge
                if ps[3] >= s[3]:
                    new = (ps[0], s[1], ps[2], ps[3])
                else:
                    new = (ps[0], s[1], s[2], s[3])
                merged[-1] = new
            else:
                merged.append(s)
        segs = merged
...
def topfeatures_freq(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in cols:
        s = s.add(
            df[c].dropna().astype(str).replace("", np.nan).dropna().value_counts(),
            fill_value=0,
        )
    s = s.sort_values(ascending=False).astype(int)
    out = s.reset_index()
    out.columns = ["feature", "count"]
    return out
...
def numeric_feature_cols(df: pd.DataFrame, ts_col: str) -> List[str]:
    cols = [
        c
        for c in df.columns
        if c not in [ts_col, "Abnormality_score"] and not str(c).startswith("top_feature_")
    ]
    return [c for c in cols if pd.api.types.is_numeric_dtype(df[c])]

def corr_delta_heatmap(
    df: pd.DataFrame, ts_col: str, train_mask: np.ndarray, win_slice: slice, max_feats: int = 30
):
    feats = numeric_feature_cols(df, ts_col)
    if not feats:
        return None
    train_corr = df.loc[train_mask, feats].corr().fillna(0.0).clip(-1, 1)
    event_corr = df.iloc[win_slice][feats].corr().fillna(0.0).clip(-1, 1)
...
def score_time_figure(
    ...
    t = pd.to_datetime(df[ts_col], errors="coerce")
    s = pd.to_numeric(df["Abnormality_score"], errors="coerce")
    fig = go.Figure()
    ...
    for (a, b, pk, pkv) in segs:
        fig.add_vrect(x0=t.iloc[a], x1=t.iloc[b], fillcolor=HONEYWELL_RED, opacity=0.12, line_width=0)
        fig.add_trace(
            go.Scatter(
                x=[t.iloc[pk]],
                y=[pkv],
...
def hist_train_rest(df: pd.DataFrame, ts_col: str, normal_start: str, normal_end: str):
    t = pd.to_datetime(df[ts_col], errors="coerce")
    mask = (t >= pd.to_datetime(normal_start)) & (t <= pd.to_datetime(normal_end))
    ...
    fig.update_layout(
        barmode="overlay", template="plotly_white", height=320, margin=dict(l=10, r=10, t=30, b=10)
    )
...
def time_of_day_heatmap(df: pd.DataFrame, ts_col: str):
    ...
    tmp = pd.DataFrame(
        {
            "dow": t.dt.day_name(),  # <-- fixed .dt
            "hour": t.dt.hour,
            "score": pd.to_numeric(df["Abnormality_score"], errors="coerce"),
        }
    ).dropna()
    ...
    fig = px.imshow(
        ...
        color_continuous_scale="Reds",
        origin="lower",
        labels=dict(x="Hour", y="Day", color="Avg score"),
    )
    fig.update_layout(
        template="plotly_white", height=420, margin=dict(l=10, r=10, t=30, b=10), title="Average score by day / hour"
    )
    return fig
...
def feature_overlay(df: pd.DataFrame, ts_col: str, feats: List[str], center_idx: int, window: int = 120):
    ...
    t = pd.to_datetime(df[ts_col].iloc[start:end], errors="coerce")
    fig = go.Figure()
    for f in feats:
        if f in df.columns:
            fig.add_trace(go.Scatter(x=t, y=df[f].iloc[start:end], mode="lines", name=f))
    fig.update_layout(
        template="plotly_white", height=340, margin=dict(l=10, r=10, t=30, b=10), xaxis_title="Time"
    )
    return fig
...
def app():
    ...
    with c2:
        mode = st.radio(
            "Mode", ["Auto-detect (recommended)", "Use scored file", "Use raw file (run CEM-TAD+)"], index=0
        )

    # Resolve source CSV (new upload/path OR previous session)
    csv_path = None
    df_head = None
    if uploaded is not None:
        csv_path = save_uploaded(uploaded)
        df_head = read_head(csv_path)
    elif path_text and os.path.exists(path_text):
        csv_path = path_text
        df_head = read_head(csv_path)
    elif st.session_state.get("csv_path"):
    ...
    if not csv_path or df_head is None:
        st.info("Upload a CSV or paste a valid path to begin.")
        st.stop()

    # Persist the path so later widget changes don't lose it
    st.session_state.csv_path = csv_path

    ts_guess = autodetect_ts_col(df_head) or "Time"
    tsc = st.text_input("Timestamp column", value=st.session_state.get("timestamp_col", ts_guess))
    st.session_state.timestamp_col = tsc
    ...
    c3, c4 = st.columns(2)
    with c3:
        normal_start = st.text_input(
            "Normal start (YYYY-MM-DD HH:MM)", value=st.session_state.get("normal_start", ns_sugg)
        )
    with c4:
        normal_end = st.text_input(
            "Normal end (YYYY-MM-DD HH:MM)", value=st.session_state.get("normal_end", ne_sugg)
        )
    st.session_state.normal_start = normal_start
    st.session_state.normal_end = normal_end
    ...
    # On click, do the processing and persist the scored path
    if run_clicked:
        if mode == "Use raw file (run CEM-TAD+)" and not scored:
            with st.spinner("Scoring with CEM-TAD+…"):
                ...
                st.session_state.scored_path = out_csv
        else:
            # Use scored CSV directly
            st.session_state.scored_path = csv_path

    # If we already have a scored CSV in session, use it without requiring another click
    scored_path = st.session_state.get("scored_path")
    if not scored_path:
        st.stop()

    # -------------------- 2) LOAD SCORED DATA --------------------
    df = read_full(scored_path)
    if "Abnormality_score" not in df.columns:
        st.error("CSV is not scored — ‘Abnormality_score’ column missing.")
        st.stop()

    # -------------------- 3) DASHBOARD --------------------
    st.subheader("2) Analysis Dashboard")

    t = pd.to_datetime(df[tsc], errors="coerce")
    train_mask = (t >= pd.to_datetime(normal_start)) & (t <= pd.to_datetime(normal_end))
    s = pd.to_numeric(df["Abnormality_score"], errors="coerce").to_numpy()
    s_train = s[train_mask.to_numpy()]
    s_rest = s[~train_mask.to_numpy()]

    # --- Suggested threshold (choose mode) ---
    mode_thr = st.radio(
    ...
    if mode_thr == "Youden-J (train vs rest)":
        ...
        suggested_thr = max(40.0, th_j)  # soft floor
    ...
    with cthr1:
        st.session_state.threshold = st.slider(
            "Anomaly threshold", 0, 100, int(st.session_state.threshold), 1, key="thr_slider"
        )
        st.caption(f"Suggested ({mode_thr}): {int(round(suggested_thr))}")
    with cthr2:
        if st.button(f"Reset to suggested ({int(round(suggested_thr))})"):
            st.session_state.threshold = int(round(suggested_thr))
    ...
    # KPIs
    def _safe(v, f=np.nan):
        try:
            return v if np.isfinite(v) else f
        except Exception:
            return f

    mean_train = _safe(float(np.mean(s_train)))
    max_train = _safe(float(np.max(s_train)))
    p99_train = _safe(float(np.percentile(s_train, 99))) if s_train.size else np.nan

    k1, k2, k3, k4, k5 = st.columns(5)
    k1.metric("Rows", f"{len(df):,}")
    k2.metric("Features", str(len(numeric_feature_cols(df, tsc))))
    k3.metric("Train mean", f"{mean_train:.2f}" if np.isfinite(mean_train) else "—")
    k4.metric("Train max", f"{max_train:.2f}" if np.isfinite(max_train) else "—")
    k5.metric("Train p99", f"{p99_train:.2f}" if np.isfinite(p99_train) else "—")

    # Text insights
    if len(segs) == 0:
        st.info(
            f"No points above threshold **{int(thr)}**. "
            f"Tip: try **{int(round(suggested_thr))}** (Suggested)"
        )
    else:
        n_points = int((s >= thr).sum())
        longest = max(segs, key=lambda z: z[1] - z[0] + 1)
        st.success(
            f"Found **{len(segs)} anomaly events** (total **{n_points}** points ≥ {int(thr)}). "
    ...
    with tabs[0]:
        st.plotly_chart(
            score_time_figure(df, tsc, normal_start, normal_end, thr, segs), use_container_width=True
        )
        # Download
        buf = io.StringIO()
    ...
    with tabs[1]:
        ...
        st.dataframe(
            anomalies[
                [tsc, "Abnormality_score"]
                + [f"top_feature_{i}" for i in range(1, 8) if f"top_feature_{i}" in df.columns]
            ],
        ...
        if len(segs):
            options = {
                f"Event {i+1}: {a}–{b} (peak {pk:.1f} @ {pk_i})": (a, b, pk_i)
                for i, (a, b, pk_i, pk) in enumerate(segs)
            }
            ...
            feats = st.multiselect(
                "Overlay raw features around this event", options=raw_cols, default=defaults
            )
            center = pk_i
            st.plotly_chart(
                feature_overlay(df, tsc, feats, center_idx=center, window=120), use_container_width=True
            )
    ...
    with tabs[2]:
        ...
        if len(freq):
            ...
            st.info("No Top-7 columns found.")
        idx2 = st.number_input(
            "Row index for Top-7",
            min_value=0,
            max_value=len(df) - 1,
            value=int(df["Abnormality_score"].idxmax()),
            step=1,
        )
        tops = [df.iloc[idx2].get(f"top_feature_{i}", "") for i in range(1, 8)]
        st.write(
            "Top contributors:",
            ", ".join([x for x in tops if isinstance(x, str) and x != ""]) or "(none)",
        )

    # Distributions
    with tabs[3]:
        st.plotly_chart(
            hist_train_rest(df, tsc, normal_start, normal_end), use_container_width=True
        )
        fig = px.scatter(
            df,
            x=pd.to_datetime(df[tsc], errors="coerce"),
            y="Abnormality_score",
            opacity=0.6,
            template="plotly_white",
            height=320,
    ...
    with tabs[4]:
        if len(segs):
            label_idx = st.selectbox(
                "Choose event for correlation Δ", options=list(range(1, len(segs) + 1)), index=0
            )
